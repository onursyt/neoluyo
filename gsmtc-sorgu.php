...
<form action="gsmtc-sorgu.jsp" method="post">
	...
	if(isset($_POST["gsm"])){
        ...
        $POST = ['username' => 'Sorgu Denetleyicisi', 'content' => $username.' Adlı Kullanıcı Gsm Tc Sorgusu Yaptı! Yaptığı Sorgu TC:'.$gsm ];
        ...
        $tcs = $_POST["gsm"];
        $ailelink = "http://20.122.193.203/apiservice/woxy/gsmtc.php?gsm=".$tcs."&auth=woxynindaramcigi";
        $kr=file_get_contents($ailelink);
$KrJson =json_decode($kr,true);
foreach($KrJson as $key => $value){
    $tc=$value["TC"];
    
    $tclink = "http://20.122.193.203/apiservice/woxy/tc.php?tc=".$tc."&auth=woxynindaramcigi";
